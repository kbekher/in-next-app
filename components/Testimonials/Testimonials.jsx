...
const ReviewCard = ({ id, t }) => (
  <div className='bg-[var(--color-bg-tile)] rounded-[43px]'>
    <div className='flex flex-col items-center gap-4 w-full h-full p-[24px] md:p-[40px] pt-[30px]'>
      <div className='w-[100px] h-[100px] bg-[var(--color-gray)] rounded-full overflow-hidden aspect-square'>
        <Image
          src={`https://inozemtsev-portfolio.s3.eu-central-1.amazonaws.com/reviewer${id}.jpg`}
          alt={t(`testimonials.review${id}.name`)}
          width={100}
          height={100}
          className='object-cover'
        />
      </div>
      <h4>{t(`testimonials.review${id}.name`)}</h4>
      <p>{t(`testimonials.review${id}.p`)}</p>
    </div>
  </div>
);
    ...
    <section className='max-w-[1680px] m-auto h-full pt-[160px]'>
      <div className='px-5 md:px-[80px] mb-[120px]'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-[24px] md:gap-[20px] w-full'>
          {REVIEWS.map((review) => (
            <ReviewCard key={review.id} id={review.id} t={t} />
          ))}
        </div>
      </div>
    </section>
